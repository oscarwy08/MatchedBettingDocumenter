...
def test_apply_nested_zip_onto_flat_install(tmp_path: Path, monkeypatch):
    ...
    (dest / "allow-firewall.bat").write_text("old\n", encoding="utf-8")
    (dest / "allow-firewall.sh").write_text("old\n", encoding="utf-8")
    (staging / "data").mkdir()
    (staging / "data" / "app.db").write_text("should-not-copy", encoding="utf-8")
    ...
    assert not (dest / "allow-firewall.bat").exists()
    assert not (dest / "allow-firewall.sh").exists()
