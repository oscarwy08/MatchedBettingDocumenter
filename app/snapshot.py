    ...
    for row in payload.get("offers", []):
        session.add(
            Offer(
                ...
                end_by=_parse_d(row.get("end_by")),
                casino_wager=_dec(row.get("casino_wager")) or Decimal("0"),
                casino_rtp=_dec(row.get("casino_rtp")) or Decimal("0"),
                spin_count=int(row.get("spin_count") or 0),
                spin_value=_dec(row.get("spin_value")) or Decimal("0"),
                spin_game=row.get("spin_game") or "",
                spin_rtp=_dec(row.get("spin_rtp")) or Decimal("0"),
                wagering_multiplier=_dec(row.get("wagering_multiplier")) or Decimal("0"),
                max_cashout=_dec(row.get("max_cashout")) or Decimal("0"),
                bonus_rtp=_dec(row.get("bonus_rtp")) or Decimal("0"),
                notes=row.get("notes") or "",
                created_at=_parse_dt(row.get("created_at")) or datetime.now(),
            )
        )
    ...
    for row in payload.get("bets", []):
        ...
        session.add(
            Bet(
                ...
                rtp=_dec(row.get("rtp")) or Decimal("0"),
                spin_count=int(row.get("spin_count") or 0),
                wagering_multiplier=_dec(row.get("wagering_multiplier")) or Decimal("0"),
